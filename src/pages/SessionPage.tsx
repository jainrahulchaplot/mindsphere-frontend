...
export default function SessionPage() {
  ...
  // Cleanup: Stop all audio when component unmounts or page changes
  useEffect(() => {
    return () => {
  ...
  const handleSessionEnd = () => {
    // Show feedback modal only once per session
    if (!hasShownFeedback) {
  ...
  const handleFeedbackSubmit = async (feedback: { rating: number; text: string }) => {
    try {
      const { api } = await import('../api/client');
      const response = await api.post(`/session/${sessionId}/feedback`, {
        post_rating: feedback.rating,
        post_feedback: feedback.text,
      });
      ...
      // Still navigate back even if feedback fails
      navigate('/');
    }
  };
